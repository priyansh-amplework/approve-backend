...
GENERATION_SCHEDULE = {
    "dad_posts": [
        ...
        {"days": [0], "hour": 11, "minute": 43, "theme": "Problem framing"},
        # TUESDAY
        {"days": [1], "hour": 16, "minute": 13, "theme": "Did you know?"},
...
def get_next_prompt():
    ...
    prompt = DAD_IMAGE_PROMPTS[index % len(DAD_IMAGE_PROMPTS)]
    post_concept = POST_CONCEPTS[index % len(POST_CONCEPTS)]

    with open(PROMPT_FILE, "w") as f:
        f.write(str((index + 1) % len(DAD_IMAGE_PROMPTS)))

    log(f"🧠 Using DAD image prompt #{index + 1}")
    log(f"📝 Post concept: {post_concept}")
    return prompt, post_concept
...
def run():
    ...
    log("🖼️ Generating image posts (not videos)")
    ...
    log(f"📋 Generating {len(pending_slots)} image post(s):")
    for hour, minute, theme in pending_slots:
        log(f"   • Slot: {hour:02d}:{minute:02d} ET - Theme: {theme}")

    # Get next prompt and post concept
    prompt, post_concept = get_next_prompt()

    # Generate base image using Gemini 2.5 Flash Image (without QR code)
    log(f"🎨 Generating base image")
    base_image = generate_image(prompt)
    if not base_image:
        log("❌ Stopping: base image not generated")
        return

    # Generate QR code with Amazon book link
    qr_code = generate_qr_code(AMAZON_BOOK_LINK)
    if not qr_code:
        log(f"⚠️ QR code generation failed, using image without QR")
        final_image = base_image
    else:
        # Add QR code to image
        final_image = add_qr_to_image(base_image, qr_code)
        if not final_image:
            log(f"⚠️ Failed to add QR code, using base image")
            final_image = base_image
        else:
    ...
    gdrive_result = upload_to_gdrive(
        file_path=final_image, folder_id=GDRIVE_FOLDER_ID, caption=caption
    )

    if gdrive_result:
        log(f"\n✅ Successfully uploaded to Google Drive!")
        log(f"   📄 Image: {gdrive_result['name']}")
        log(f"   🔗 Link: {gdrive_result['link']}")
        log(f"   🔲 QR Code: Amazon book link embedded")
        ...
        log(f"   Image saved locally: {final_image}")

    # Cleanup local files
    if os.path.exists(base_image):
        os.remove(base_image)
    if final_image != base_image and os.path.exists(final_image):
        os.remove(final_image)
    log("🗑 Cleaned up local image files")

    log("\n" + "=" * 60)
    log("✅ GENERATION COMPLETE - DAD Campaign")
    log("🖼️ Image post with Ayrshare auto-generated hashtags")
